    ...
    return (
        <section className="rounded-4xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4">
                {members.map((member) => (
                    <div className="flex items-center justify-between rounded-3xl border border-slate-100 p-5 transition-all duration-300 hover:-translate-y-1 hover:border-indigo-100 hover:shadow-lg " key={member.name}>
                        <div className="relative">
                            <div className="flex items-center gap-4">
                                <span className={`absolute bottom-0 left-0 top-0 h-3.5 w-3.5 rounded-full border-2 border-white ${member.status === "Online" ? "bg-green-500" : member.status === "Away" ? "bg-yellow-500" : "bg-slate-400"}`}></span>
                                <img src={member.avatar} className="h-14 w-14 rounded-full object-cover" alt={member.name} />
